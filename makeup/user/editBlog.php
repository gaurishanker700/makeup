...
include '../db.php';
$eid = $_GET["id"];
$edit="SELECT * FROM `blog` WHERE `id`='".$eid."'";
...
if(isset($_POST["submit"])) {
    ...
    $sql="UPDATE `blog` SET `date` = '$date', `heading` = '$heading', `description` = '$description', `content` = '$content', `image` = '$folder' WHERE `id` = '$eid'";
    $result = mysqli_query($conn, $sql);
    if($result) {
        echo "<script>alert('Blog updated successfully!')</script>";
        // header("Location: http://localhost/makeup/admin/Add-blog.php");
    } else{
        die("<p>Error updating blog post: " . mysqli_error($conn). "</p>");
    }
...
<body>
        ...
        <div class="row">
            ...
            <div class="col-lg-3 sidebar">
                ...
                <ul class="nav flex-column">
                    ...
                    <li class="nav-item">
                      <a class="nav-link" href="#"><i class="fas fa-eye"></i> View Courses</a>
                    </li>
                    <li class="nav-item">
    ...
    <form action="#" method="POST" enctype="multipart/form-data" style="background-color: violet; padding: 20px; border-radius: 10px;">
        <div class="mb-3">
            ...
            <input type="date" class="form-control" value="<?php echo $row['date']; ?>" id="date" name="date" required style="border-color: pink;">
        </div>
        <div class="mb-3">
        ...
        <div class="mb-3">
            ...
            <textarea class="form-control" id="blogContent" name="content" rows="6" required style="border-color: pink;"><?php echo $row['content']; ?></textarea>
        </div>
